...
import { FlaskConical, Search, Edit, Trash2, X, Plus, Save, DollarSign, Clock, ChevronLeft, ChevronRight } from 'lucide-react';
...
function ManageXetNghiem() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter(
      (item) =>
        item.tenXN?.toLowerCase().includes(searchTerm) ||
        item.maXN?.toLowerCase().includes(searchTerm) ||
        item.maLoaiXN?.toLowerCase().includes(searchTerm)
    );
  }, [list, search]);
  ...
  const paginatedXetNghiem = useMemo(() => {
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-200">
                {paginatedXetNghiem.map((item, idx) => (
