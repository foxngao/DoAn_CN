...
import { Ruler, Search, Edit, Trash2, X, Plus, Save, ChevronLeft, ChevronRight } from 'lucide-react';
...
const QuanLyDonViTinh = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter(
      (item) =>
        item.tenDVT?.toLowerCase().includes(searchTerm) ||
        item.maDVT?.toLowerCase().includes(searchTerm) ||
  ...
  const paginatedDonViTinh = useMemo(() => {
