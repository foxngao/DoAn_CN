...
import { Package, Search, Edit, Trash2, X, Plus, Save, ChevronLeft, ChevronRight } from 'lucide-react';
...
const QuanLyNhomThuoc = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter(
      (item) =>
        item.tenNhom?.toLowerCase().includes(searchTerm) ||
        item.maNhom?.toLowerCase().includes(searchTerm) ||
  ...
  const paginatedNhomThuoc = useMemo(() => {
