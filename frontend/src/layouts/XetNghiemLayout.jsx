...
const SidebarItem = ({ to, icon: Icon, label }) => {
  ...
  const isActive = location.pathname === to || (to !== '/xetnghiem' && location.pathname.startsWith(to));
...
const XetNghiemLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      >
        {/* Header */}
        <div className="h-20 flex items-center justify-between px-6 border-b border-indigo-800 bg-indigo-900">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl shadow-lg ring-2 ring-indigo-500/30">
              <TestTube size={24} />
            </div>
            <div>
              <h1 className="font-bold text-lg leading-tight tracking-tight">SmartHospital</h1>
              <span className="text-xs text-indigo-300 font-medium tracking-wide">XÉT NGHIỆM</span>
            </div>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
        ...
        {/* Menu Items */}
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-2 scrollbar-thin scrollbar-thumb-indigo-700 scrollbar-track-indigo-900">
          <MenuSection title="Tổng quan">
            <SidebarItem to="/xetnghiem" icon={LayoutDashboard} label="Trang chủ" />
          </MenuSection>

          <MenuSection title="Quản lý xét nghiệm">
            <SidebarItem to="/xetnghiem/xetnghiem/yeucau" icon={FlaskConical} label="Yêu cầu xét nghiệm" />
            <SidebarItem to="/xetnghiem/xetnghiem/phieu" icon={FileText} label="Phiếu xét nghiệm" />
          </MenuSection>
        </div>

        {/* Footer - User Info & Logout */}
        <div className="p-4 border-t border-indigo-800 bg-indigo-900">
          <div className="bg-indigo-800/50 rounded-xl p-4 mb-3">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center border-2 border-indigo-600">
                <UserCircle size={20} className="text-white" />
              </div>
              <div className="overflow-hidden flex-1">
                <p className="text-sm font-bold truncate text-white">{nhanSuInfo?.hoTen || 'Nhân viên xét nghiệm'}</p>
                <p className="text-xs text-indigo-300 truncate">
                  {nhanSuInfo?.chuyenMon || 'Xét nghiệm'}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative bg-gray-50">
        ...
        <header className="bg-white shadow-sm border-b border-gray-200 h-16 flex items-center px-4 md:hidden z-10">
          ...
          <span className="ml-3 font-bold text-gray-800 text-lg">Xét nghiệm</span>
        </header>
